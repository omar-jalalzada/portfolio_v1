...
x$(document).on("DOMContentLoaded", function() {
  ...
  x$('.project_wrapper').each(function () {
  ...
  x$('#subnav ul').find('li').each( function(nav) {
    var wc = x$('#p_wrapper_class')[0].innerHTML
    var cl = x$(nav).attr('class')
     if ( wc == cl ) {
      x$(this).find('a').addClass('active');
     }
...
// Cookies snippit from http://www.quirksmode.org/js/cookies.html
...
x$(document).on("DOMContentLoaded", function() { 
  ...
  var is_andriod = (agent.indexOf('android')!=-1);

  if ( is_iphone |is_ipad | is_andriod) {
  
  
    x$('body.home_page').each(function () {
      
      x$(".box").each(function(boxes){
        x$(boxes).addClass("touch");
      });
